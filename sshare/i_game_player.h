...
public:
	...
	virtual bool change_gold(GOLD_TYPE cgold ,GOLD_TYPE commission =0) = 0;
	//修改礼券
	...
	//@设置奖券;
	virtual bool change_chip(int chip) = 0;
	...
	//@获取一条玩家数据;
	virtual std::map<int ,std::shared_ptr<tagBagItem>>*  getitem() =0;

	//@获取游戏配置信息;
	virtual tagStaticOp* GetOpItems() = 0;

	//@获取玩家地域信息;
	virtual std::string *GetUserRegion() = 0;

	//@如果背包属性改变,通知world;
	virtual bool change_bagitem(const tagBagItem & item) = 0;
	...
	//@发送请求离开报文;
	//@请求world清理玩家信息,相当于正常退游戏;
	virtual void reqPlayer_leaveGame(int status=0) = 0;
