...
{
	DragonTiger_RobotCFG::GetSingleton()->Load();
	DragonTiger_RoomCFG::GetSingleton()->Load();
	DragonTiger_BaseInfo::GetSingleton()->Load();
	DragonTiger_CardsCFG::GetSingleton()->Load();
	DragonTiger_Cardodds::GetSingleton()->Load();

	//库存与权重
	DragonTiger_RoomStockCFG::GetSingleton()->Load();
	DragonTiger_WeightCFG::GetSingleton()->Load();
...
{
	init_proc_dragon_tiger_logic();
	init_proc_dragon_tiger_protocol();
}
...
void logic_lobby::init_game(int count, int room_id, int child_id)
{
	...
	const boost::unordered_map<int, DragonTiger_RoomCFGData>& list = DragonTiger_RoomCFG::GetSingleton()->GetMapData();
...
void logic_lobby::heartbeat( double elapsed )
{
	...
	if(m_check_cache > 60)
	{
		...
		SLOG_CRITICAL<<"current players count="<<m_all_players.size();
	}
}
