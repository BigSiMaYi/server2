...
const std::string& db_log::get_tablename(uint16_t table_type)
{
	...
	static const std::string ROOM_STOCK = "pumpDragonTiger_RoomStock";
...
void db_log::property_log(logic_player* player, int gameId, int ptype, GOLD_TYPE addValue, GOLD_TYPE taxValue, int cutRound, int reason, const std::string& param)
{
	...
	builder.append("roomId", player->get_room()->get_id());
...
void db_log::pumpRoomStock(logic_room* proom, double oldstock, double oldwinnertax, double losttax)
{
	mongo::BSONObjBuilder builder;
	builder.appendTimeT("genTime", time_helper::instance().get_cur_time());
	builder.append("roomId", proom->get_id());
	builder.append("gameId", game_engine::instance().get_gameid());
	builder.append("oldStock", oldstock);	
	builder.append("newStock", proom->TotalStock->get_value());	

	builder.append("oldWinnerTax", oldwinnertax);
	builder.append("newWinnerTax", proom->TotalTaxWinner->get_value());
	builder.append("oldLostTax", 0);
	builder.append("newLostTax", 0);

	builder.append("WaterConfigID", proom->get_current_warter());

	push_insert(game_log_pump_room_stock,builder.obj());
