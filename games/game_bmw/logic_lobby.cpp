...
{	
	BMW_RoomCFG::GetSingleton()->Load();
	BMW_BaseInfo::GetSingleton()->Load();
	BMW_LotteryCFG::GetSingleton()->Load();
	BMW_ResultCtrl::GetSingleton()->Load();
	BMW_RobotCFG::GetSingleton()->Load();
...
{
	init_proc_bmw_logic();
	init_proc_bmw_protocol();
}

void bmw_space::logic_lobby::init_game(int count, int32_t single_room /*= -1*/, int32_t child_room /*= -1*/)
{
	m_max_player = count;
	...
	init_config();
	// 创建游戏房间
	const boost::unordered_map<int, BMW_RoomCFGData>& list = BMW_RoomCFG::GetSingleton()->GetMapData();
	for (auto it = list.begin(); it != list.end(); ++it)
	{
		...
		{
			// 创建房间
			auto room = boost::make_shared<logic_room>(&(it->second), this,child_room);
			// 保存房间信息
...
void bmw_space::logic_lobby::robot_leave(int32_t playerid)
{
...
void bmw_space::logic_lobby::robot_enter(LPlayerPtr& player, int tag)
{
	int32_t rid = tag>>4;
	int32_t tid = tag&0x0F;
...
void logic_lobby::player_kick_player(uint32_t playerid, int bforce /*= 0*/)
{
	if (0 == bforce)
	{
		SLOG_WARNING << boost::format("Warning: 260");
	}
	else if (1 == bforce)
	{
	...
	{
		SLOG_WARNING << boost::format("Warning: 272");
	}
}
...
void logic_lobby::gmPlatformOpt(int32_t optype, int32_t gameID, int32_t roomID /*= 0*/, int32_t cutRound/* = 0*/)
{
	if (gameID == game_engine::instance().get_gameid())
	{ // 处理对应 Game_ID
		if (0 == roomID)
		{ // 处理所有房间
			for (auto room : m_rooms)
			{
				...
				{
					SLOG_WARNING << boost::format("BMW Refuse optype:%d")%optype;
				} 
			}
		}
		else
		{ // 处理指定房间
			for (auto room : m_rooms)
			{
				...
				if (roomID == room.first)
				{
					...
					{
						SLOG_WARNING << boost::format("BMW roomid:%d,Refuse optype:%d")%roomID%optype;
					}
				}
			}
		}
