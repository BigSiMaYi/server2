...
{
public:
	logic_room(const BMW_RoomCFGData* cfg, logic_lobby* _lobby,int childid);
	~logic_room(void);
	void release();
	void heartbeat( double elapsed );
	virtual uint32_t get_id();
	uint32_t get_cur_count();
	...
	const BMW_RoomCFGData* get_roomcfg();
	...
	const BMW_RoomCFGData* get_data() const;
