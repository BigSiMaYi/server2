...
{	
	...
	for (auto it = rooms.begin(); it != rooms.end(); ++it)
	{
		...
		int32_t nRoomID = it->second->get_id();
		int32_t nCount = it->second->get_cur_count();
		int32_t nMaxCount = it->second->get_max_count();
		if(nRoomID==1)
		{
			nCount = nCount*2+38;
		}
		...
		r->set_roomid(nRoomID);
		r->set_nowplayers(nCount);
		r->set_maxplayers(nMaxCount);
	}
