...
const std::string& db_log::get_tablename(uint16_t table_type)
{
	...
	static const std::string ALL_ONLINE_GAME = "AllOnlineGaming";
...
void db_log::joingame2(int playerid)
{
	mongo::BSONObjBuilder builder;
	builder.append("playerId", playerid);
	builder.appendTimeT("genTime", time_helper::instance().get_cur_time());
	push_update(8, BSON("playerId" << playerid), BSON("$set" << builder.obj()));
}

void db_log::leavegame2(int playerid)
{
	push_delete(8, BSON("playerId" << playerid));
}
