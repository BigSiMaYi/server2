...
void init_proc_landlord_logic()
{
    ...
    packetl2c_scene_info_fapai_factory::regedit_factory();
    packetl2c_scene_info_banker_factory::regedit_factory();
}
...
{	
	...
	case eGameState_End:
		{
			...
			auto sendmsg = lcplayer->get_table()->get_scene_info_msg_free(uid);
	...
	case eGameState_FaPai:
        {
            uint32_t uid = lcplayer->get_pid();
            auto sendmsg = lcplayer->get_table()->get_scene_info_msg_fapai(uid);
            if (lcplayer->is_robot())
            {
                robot_mgr::instance().recv_packet(uid, sendmsg->packet_id(), sendmsg);
            }
	...
	case eGameState_Banker:
		{
            uint32_t uid = lcplayer->get_pid();
            auto sendmsg = lcplayer->get_table()->get_scene_info_msg_banker(uid);
            if (lcplayer->is_robot())
            {
                robot_mgr::instance().recv_packet(uid, sendmsg->packet_id(), sendmsg);
            }
	...
	case eGameState_Play:
		{
			uint32_t uid = lcplayer->get_pid();
			lcplayer->get_table()->onEventAuto(uid, false);
													...
													shared_ptr<packetc2l_ask_ready> msg)
{
	...
	if(!lptable)
	{
		SLOG_ERROR<<"player ask ready id: " << lcplayer->get_pid()<<", table is null";
		return false;
	}
												   ...
												   shared_ptr<packetc2l_ask_outcard> msg)
{
    ...
    if (!player->is_robot())
    {
        ...
        for (int i = 0; i < size; ++i)
        {
            ...
            pokers.push_back(poke);
        }
        if (!pokers.empty())
        {
            lptable->onEventOutCard(player->get_playerid(), pokers);
        }
    }
