...
{	
	GoldFlower_RoomCFG::GetSingleton()->Load();
	GoldFlower_BaseInfo::GetSingleton()->Load();
	GoldFlower_RobotCFG::GetSingleton()->Load();
	GoldFlower_RobotGameCFG::GetSingleton()->Load();
	GoldFlower_RobotRoomCFG::GetSingleton()->Load();
	GoldFlower_RobotActionCFG::GetSingleton()->Load();
}
...
{
	init_proc_zjh_logic();
	init_proc_zjh_protocol();
}

void zjh_space::logic_lobby::init_game(int count, int32_t single_room /*= -1*/)
{
	m_max_player = count;
	...
	init_config();
	// 创建游戏房间
	const boost::unordered_map<int, GoldFlower_RoomCFGData>& list = GoldFlower_RoomCFG::GetSingleton()->GetMapData();
...
void logic_lobby::heartbeat( double elapsed )
{
	if(!m_init)return;
	//
	robot_mgr::instance().process_send_packet();
...
void zjh_space::logic_lobby::robot_leave(int32_t playerid)
{
...
void zjh_space::logic_lobby::robot_enter(LPlayerPtr& player, int tag)
{
	int32_t rid = tag%100;
	int32_t tid = tag/100;
