...
bool game_engine::init_engine(enable_xml_config& config)
{
	...
	SLOG_CRITICAL << "game_engine::init_engine zjh";
...
void game_engine::init_db(enable_xml_config& xml_cfg , int32_t gameID, int32_t roomID, int32_t child_roomID)
{
	...
	{
		auto gdburl = xml_cfg.get<std::string>("gamedb_url");
		auto gdbname = xml_cfg.get<std::string>("gamedb_name");
		db_game::instance().init_db(gdburl, gdbname);
	}

	if(xml_cfg.check("logdb_url") && xml_cfg.check("logdb_name"))
	{
		auto lgdburl = xml_cfg.get<std::string>("logdb_url");
		auto lgdbname = xml_cfg.get<std::string>("logdb_name");
		db_log::instance().init_db(lgdburl, lgdbname);
		//@add by Hunter 2017/11/24;
...
void game_engine::gmPlatformOpt(int32_t optype, int32_t gameID, int32_t roomID /*= 0*/, int32_t cutRound /*= 0*/, int32_t exData1 /*= 0*/, int32_t exData2 /*= 0*/)
{
	if (gameID == get_gameid()) {
		SLOG_CRITICAL << boost::format("gmPlatformOpt optype:%1%,roomID:%2%,cutRound:%3%,exData1:%4%,exData2:%5%") 
			%optype%roomID%cutRound%exData1%exData2;
		// 广播消息
		m_lobby.gmPlatformOpt(optype, gameID, roomID, cutRound,exData1,exData2);
