...
public:
	...
	void init_game(int count, int32_t single_room = -1);
	void release_game();
	...
	void player_clean_player(uint32_t playerid);
	...
	void gmPlatformOpt(int32_t optype, int32_t gameID, int32_t roomID = 0, int32_t cutRound = 0, int32_t exData1 = 0, int32_t exData2 = 0);
private:
	void init_config();
	void init_protocol();
	void reload_fishcfg();
