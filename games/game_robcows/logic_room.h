...
{
public:
	logic_room(const RobCows_RoomCFGData* cfg, logic_lobby* _lobby);
	~logic_room(void);
	void release();
	...
	void cleanplayer(uint32_t pid);
	const RobCows_RoomCFGData* get_roomcfg();
	...
	const RobCows_RoomCFGData* get_data() const;
...
private:
	logic_lobby* m_lobby;
	const RobCows_RoomCFGData* m_cfg;

	RobCows_RobotRoomCFGData m_robotcfg;
	LTABLE_MAP m_tables;
