...
bool logic_player::store_game_object(bool to_all)
{
	...
	auto err = db_game::instance().update(DB_RED_BLACK_PLAYER, BSON("player_id" << get_pid()), BSON("$set" << to_bson(to_all)));
	if (!err.empty())
	{
...
#if 0
				//广播
				static int broadcast_gold = RedBlack_BaseInfo::GetSingleton()->GetData("BroadcastGold")->mValue;
				if (m_change_gold >= broadcast_gold)
				{
					const std::string *pStr = m_player->getLan("RedBroadcast");
					if (pStr)
					{
...
GOLD_TYPE  logic_player::get_bet_gold()
{
...
int  logic_player::get_winner_count()
{
	int winner_count = 0;
...
std::list<GOLD_TYPE> &  logic_player::get_bet_gold_list()
{
	return m_list_bet_gold;
}

std::list<bool>&  logic_player::get_winner_list()
{
...
bool logic_player::check_bet_gold(GOLD_TYPE bet_gold, GOLD_TYPE total_gold)
{
	...
	static int betRate = RedBlack_BaseInfo::GetSingleton()->GetData("BetRate")->mValue;
...
void logic_player::add_star_lottery_info(int32_t award,int32_t star /*= 0*/)
{
	...
	static int WinStarCount = RedBlack_BaseInfo::GetSingleton()->GetData("WinStarCount")->mValue;
...
bool logic_player::check_room_gold(GOLD_TYPE& room_gold)
{
	auto pid = get_pid();
	room_gold = m_room->get_data()->mBetGoldCondition;
	if (get_gold() >= room_gold)
	{
		return true;
	}
	else
...
void  logic_player::store_balance_record()
{
...
void  logic_player::clear_balace_info()
{
	m_balance_bet_gold = 0;
	m_balance_win_or_lose = 0;
	m_win_lose_final_gold = 0;
...
{
	mongo::BSONObj b = db_game::instance().findone(DB_RED_BLACK_PLAYER, BSON("player_id" << get_pid()));
	if (b.isEmpty())
		return false;
