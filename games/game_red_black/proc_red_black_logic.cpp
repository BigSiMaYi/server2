...
{	
	...
	if(lcplayer && lcplayer->get_room())
	{
		...
		GOLD_TYPE room_gold_cond = 0;
		lcplayer->check_room_gold(room_gold_cond);
		scene_info->set_room_gold_cond(room_gold_cond);
...
// 			banker_info->set_snatch_player_id(game_main->get_snatch_player());
		...
		if (state >= (int)logic_main::game_state::game_state_deal)
		{
			...
			game_main->fill_scene_info(scene_info);
...
{	
	...
	if(lcplayer && lcplayer->get_room())
	{
		...
		GOLD_TYPE room_gold_cond = 0;
		if (lcplayer->check_room_gold(room_gold_cond))
		{
			...
			sendmsg->set_room_gold_cond(0);
		}
		else
		{
			int ret = msg_type_def::e_msg_result_def::e_rmt_topup_not_enough;
			if (!msg_type_def::e_msg_result_def_IsValid(ret))
			{
				SLOG_ERROR << "e_rmt_topup_not_enough value error";
				ret = msg_type_def::e_msg_result_def::e_rmt_unknow;
			}

			sendmsg->set_result((msg_type_def::e_msg_result_def)ret);
			sendmsg->set_room_gold_cond(room_gold_cond);
		}
...
{	
	...
	if(lcplayer && lcplayer->get_room())
	{
		...
		sendmsg->set_red_win_counts(lcplayer->get_room()->get_red_win_counts());
		sendmsg->set_black_win_counts(lcplayer->get_room()->get_black_win_counts());

		auto& history_infos = lcplayer->get_room()->get_history_pokes_info();
		sendmsg->mutable_history_infos()->Reserve(history_infos.size());

		for (auto& it : history_infos)
		{
			auto cards_trend = sendmsg->add_history_infos();
			cards_trend->set_win_type(it.first);
			cards_trend->set_win_area(it.second);
		}
